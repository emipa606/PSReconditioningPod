...
namespace PS_ReconPod 
{
    public class PS_Hediff_Reconditioned : Hediff
    {
        ...
        {
            this.def = DefDatabase<HediffDef>.GetNamed("PS_Hediff_Reconditioned");
            this.CheckedConData = false;
            this.ConditioningDataList = new List<PS_Conditioning_Data>();
        }
        ...
        public override void ExposeData()
        {
            ...
            Scribe_Collections.Look<PS_Conditioning_Data>(ref this.ConditioningDataList, "ConditionDataList");
        ...
        public override string LabelBase
        {
            get
            {
                ...
                //switch (this.ConditioningData.AlterType)
                //{
                //    case TraitAlterType.Added:
                //        return "PS_HediffReconditionedAddedLab".Translate() + " " + this.ConditioningData.AddedTraitLabel;

                //    case TraitAlterType.Removed:
                //        return "PS_HediffReconditionedRemovedLab".Translate() + " " + this.ConditioningData.OrigonalTraitLabel;

                //    case TraitAlterType.Altered:
                //        return "PS_HediffReconditionedAlteredLab".Translate() + " " + this.ConditioningData.OrigonalTraitLabel;
        ...
        {
            if (this.ConditioningData != null)
            {
                if (ConditioningDataList == null || !ConditioningDataList.Any())
                {
                    Log.Message(string.Format("PS_Hediff_Reconditoning: Condition data for {0} found as single and not list. This is due to multi-condition update. Building list now.", this.pawn.LabelShort));
                    this.ConditioningDataList = new List<PS_Conditioning_Data>();
                ...
                {
                    Log.Message(string.Format("PS_Hediff_Reconditoning: Condition data for {0} found as single and not list due to update. But list was already occupied.", this.pawn.LabelShort));
                    ConditioningDataList.Add(new PS_Conditioning_Data
                    {
                        PawnId = ConditioningData.PawnId,
                        AddedTraitDefName = ConditioningData.AddedTraitDefName,
                ...
                tempList = this.ConditioningDataList.Where(x => x.IsValid()).ToList();
        ...
        {
            base.Tick();
            if(!this.CheckedConData)
            {
                this.CheckForOldConData();
                this.CheckedConData = true;
            }
        }
    }
}
