...
public class PS_Panel_Reconditioning : Window
{
    ...
    public override void DoWindowContents(Rect inRect)
    {
        ...
        var searchRect = new Rect(((windowRect.width - (Margin * 2f)) * 0.75f) + 3f,
            CurrentTraitScrollView.DrawRect.y - 23f, windowRect.width * 0.20f, 20f);
        searchString = Widgets.TextField(searchRect, searchString);
        TooltipHandler.TipRegion(searchRect, "PS_Search".Translate());
    ...
    private void Init()
    {
        ...
        CurrentTraits = CurrentTraits.OrderBy(trait => trait.LabelCap).ToList();
    ...
    public string GetSelectedString()
    {
        ...
        if (ChangeType is TraitAlterType.Added or TraitAlterType.Altered)
        {
            return ToAdd.Label;
        }
